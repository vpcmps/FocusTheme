...
TEMPLATE = """<Themes>
  <!-- %(name)s
       ...
       Syntax:   keyword #%(keyword)s, method #%(method)s, variable #%(variable)s,
                 property #%(property)s, operator #%(operator)s,
                 string #%(string)s, number #%(number)s
       Accent:   #%(accent)s
  ...
  <Theme Name="%(name)s" GUID="%(guid)s" FallbackId="%(fallback)s">
    ...
    <Category Name="Text Editor MEF Items" GUID="{75a05685-00a8-4ded-bae5-e7a50bfa929a}">
      ...
      <!-- Names: two hues, split by where the name lives. The design mockups
           leave identifiers at plain text, but that collapses every name into
           the same read as punctuation and operators; giving them a hue is what
           makes "what is this name" answerable at a glance.

           Locals and parameters take `variable`, a desaturated plain text - they
           are the bulk of what is on a line and should read as text. Members
           declared on a type and reached through a dot - property, field, event
           - take `property`, a hue of its own, so `dados.Cpf` says which of the
           two `Cpf` is without having to look up the declaration. -->
      <Color Name="local name">
        <Foreground Type="CT_RAW" Source="FF%(variable)s"/>
      </Color>
      <Color Name="field name">
        <Foreground Type="CT_RAW" Source="FF%(property)s"/>
      </Color>
